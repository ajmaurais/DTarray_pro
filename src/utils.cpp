...
bool utils::isAbsPath(const std::string& _fname){
	return _fname[0] == '/';
}
bool utils::isAbsPath(const char* _fname){
	return _fname[0] == '/';
}
...
bool utils::mkdir(const char* path)
{
	...
	//make dir
	utils::systemCommand("mkdir " + rpath);
...
void utils::removeBlanks(std::vector<std::string>& elems)
{
